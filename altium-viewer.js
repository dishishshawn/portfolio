window.ProjectPcbViewer = (() => {
  ...
  const normalizeActiveView = (value, enabledViews) => {
    const activeView = String(value || DEFAULT_ACTIVE_VIEW)
      .trim()
      .toLowerCase();
  ...
  const needsPublicHost = (viewer = {}) =>
    isLocalHost() &&
    (Boolean(viewer.requiresPublicHost) || isLocalUrl(viewer.projectSrc));
  ...
  const renderAction = (viewer, mode) => {
    if (viewer.openUrl) {
      return `
                <a href="${escapeHtml(viewer.openUrl)}" target="_blank" rel="noreferrer" class="btn btn-solid">
    ...
    const label =
      mode === "fallback" ? "Viewer source needed" : "Live on this page";
    return `<span class="btn btn-quiet viewer-disabled" aria-disabled="true">${label}</span>`;
  };

  const renderViewerSurface = (
    viewer,
    title,
    enabledViews,
    activeView,
  ...
  const render = (viewer = {}) => {
    ...
    const caption = escapeHtml(viewer.caption || "");
  ...
  const init = (root = document) => {
    root.querySelectorAll("[data-altium-viewer]").forEach((container) => {
      ...
      initViewerFrame(
        container,
        ".altium-web-viewer, .viewer-embed-target iframe",
      );
    });
  };
